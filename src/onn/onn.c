...
void device_init() {
  if(onn_device_object) return;
  onn_set_evaluators("eval_device", evaluate_device_logic, 0);
  onn_device_object=object_new(0, "eval_device", "device", 8);
  object_set_cache(onn_device_object, "keep-active");
}
...
void start_timer_for_soonest_timeout_if_in_future() {
  ...
  if(soonest){
    ...
    if(t>=0) time_once(just_wake_up, 0, t);
  }
}
...
char* object_to_text(object* n, char* b, uint16_t s, int target) {
  ...
  if(target==OBJECT_TO_TEXT_NETWORK){
    ln+=snprintf(b+ln, s-ln, " Devices: %s", value_string(onn_device_object->uid)); BUFCHK
  }
...
char* object_uid_to_text(char* uid, char* b, uint16_t s, int style){
  return object_to_text(onn_get_from_cache(uid), b, s, style);
}
...
bool onn_loop() {
  bool lka=false, pka=false, oka=false, eka=false;
#if defined(LOG_ONN_LOOP)
  uint32_t zt=time_ms();
#endif
  lka = log_loop();
#if defined(LOG_ONN_LOOP)
...
#if defined(LOG_ONN_LOOP)
  log_write("l=%ld p=%ld o=%ld e=%ld\n", lt-zt, pt-lt, ot-pt, et-ot);
...
object* onn_get_from_cache(char* uid) {
...
void onn_set_evaluators(char* name, ...) {
  ...
  onn_evaluator evaluator;
  va_list valist;
  va_start(valist, name);
  while((evaluator = va_arg(valist, onn_evaluator))){
    list_add(evals, evaluator);
  }
  va_end(valist);
}

void onn_run_evaluators(char* uid, void* data){
  if(!uid) return;
  object* o=onn_get_from_cache(uid);
  if(!o) return;
...
void run_evaluators(object* o, void* data, value* alerted, bool timedout){
  ...
  for(int i=1; i<=list_size(evals); i++){
    onn_evaluator eval=(onn_evaluator)list_get_n(evals, i);
...
void onn_recv_object(object* n) {
  object* o=onn_get_from_cache(value_string(n->uid));
  if(!o){
  ...
  if(object_is_remote_device(o)){
    add_notify(o, value_string(onn_device_object->uid));
    add_notify(onn_device_object, value_string(o->uid));
  }
  save_and_notify(o);
}
