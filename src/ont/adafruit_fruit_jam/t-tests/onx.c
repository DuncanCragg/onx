...
const uint32_t startup_clockspeed_khz      = 320*1000;
const uint8_t  startup_hstx_clock_divider  = 2;
const uint8_t  startup_flash_clock_divider = 3;
const uint8_t  psram_clock_divider         = 2;
...
const int8_t led_matrix_dotstar_sck_pin = -1;
const int8_t led_matrix_dotstar_tx_pin  = -1;
...
void startup_core0_loop(){
  ...
  if(char_recvd){
    ...
    if(char_recvd=='l') run_actual_leds(false);
    // ------ same as log.c ------------
