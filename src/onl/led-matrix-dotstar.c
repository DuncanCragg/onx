...
void led_matrix_show(){
  ...
  d=0x00; for (i = 0; i < 4; i++) spi_write(&d, 1);

  // BGR not RGB!
  for(uint16_t i=0; i<num_leds; i++){
    d=0xFF; spi_write(&d, 1);
    d=led_matrix_array[i][2]/scale; spi_write(&d, 1);
    d=led_matrix_array[i][1]/scale; spi_write(&d, 1);
    d=led_matrix_array[i][0]/scale; spi_write(&d, 1);
  }

  // https://cpldcpu.wordpress.com/2014/11/30/understanding-the-apa102-superled/
  // and see Adafruit_DotStar.cpp
  d=0xFF; for (i = 0; i < ((num_leds + 15) / 16); i++) spi_write(&d, 1);
}
